...
class TestCounterMethods(unittest.TestCase):
    ...
    def test_count_file(self):
        cnt_obj = WordCount()
        self.assertEqual(handlers.FileHandler("/Users/nofardiamant/Documents/word_count/sample.txt", cnt_obj).count_distinct_words(), 319)
        cnt_obj = WordCount()
        self.assertEqual(handlers.FileHandler("/Users/nofardiamant/Documents/word_count/sample.txt", cnt_obj).count_word_occurance("sample"), 7)
...
class TestInputMethods(unittest.TestCase):
    ...
    def test_file_input(self):
        cnt_obj = WordCount()
        curr_dir = os.getcwd()
        self.assertIsInstance(InputFactory().create_input_obj(curr_dir + "/sample.txt", cnt_obj), handlers.FileHandler)
